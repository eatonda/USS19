...
struct Clunky_Event_Element **generate_ship(int length, int color, struct Clunky_Event_Element_Container *eec, struct Ship_Assets s){
    ...
    ships[0] = clunky_dragable_element_init(sprite, 0, 0, 2, "ship\0");//2

    int x, cnt = 1;

    for (x = sprite->ap_w; x < (length-1)*sprite->ap_w; x += sprite->ap_w){
        ships[cnt++] = clunky_dragable_element_init(sprite, x, 0, 8, "ship\0");//8
        
    }


    ships[length-1] = clunky_dragable_element_init(sprite, x, 0, 4, "ship\0");//4
